...
URathausSpandau = (hof, lang, smallString, content)=>{
	if (hof !== 1 && _.$.q(`[data-short=${lang}]`))
		return;
	let elCont = '';
	if (content)
		elCont = content;
	...
	if (hof === 1)
		return html;
	let el = _.$.id('langsShorts')
	el.insertAdjacentHTML('beforeend', html)
};

AmOmnibusHof = (hof, lang, smallString, content)=>{
	if (hof !== 1 && _.$.q(`[data-lang=${lang}]`))
		return;
	let elCont = '';
	if (content)
		elCont = content;
...
addFind = (channel)=>{
	...
	langList.forEach(lang=>{
		langs += 
		`<button onclick="AmOmnibusHof(0,'${lang}','${smallString}');return false" style="width:40px;margin:2px" class="emptybtn">`+
			`<img src="${helperUrl}imgs/${lang}.png" width=40px style="padding-bottom:6px">`+
		`</button>`;
	...
	.then (data=>{
		...
		if (descMap)
			for (let Lang in descMap)
				langsDescrsHtml += AmOmnibusHof(1, Lang, smallString, descMap[Lang]);
		else
			langsDescrsHtml = AmOmnibusHof(1, lang, smallString, description);
		...
		if (shortsMap)
			for (let Lang in shortsMap)
				langsShortsHtml += URathausSpandau(1, Lang, smallString, shortsMap[Lang]);
		else if (short > 0)
			langsShortsHtml = AmOmnibusHof(1, lang, smallString, short);
