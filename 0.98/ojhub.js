...
bottomNavButton = (text, Class, oncl, icon = '', matchPath = '')=>{
	// делаем сплит чтобы вложенные маршруты не ломали красоту картинки
	if (_.link.compile()[0].split('/')[0] === matchPath)
		Class += ' active'
...
bottomNav = ()=>{
	...
	bottomNavButton('projects',	'navbtn','pageFind(helperFindData[3])', navIcon('projects'), 'find')+
	bottomNavButton('guides09',	'navbtn','pageWikiList()', navIcon('guides09'), 'Wikis')+
	bottomNavButton('news',		'navbtn','globalNews()', navIcon('news'), 'news')+
	bottomNavButton('vacancies',	'navbtn','globalVacs()', navIcon('vacs'), 'vacs')+
	bottomNavButton('profile',	'navbtn','profilePage()', navIcon('profile'), 'profile')+
	`</nav>`;
},
			...
			.then(data=>{
				switch (data) {
					...
					case '-4':
						...
						break;
					default:
						let serverResp = helperInitData(data);
...
ADwrite = (userId = '')=>{
	...
	_.win.open('writeAlarm', 
		...
		`<form onsubmit="return enterFormData(this,'${sData[1]}writeAlarm${php}')">`+
...
switchLangMenu = ()=>{ // makeSwticher(0,'switchHtmlLang2', switchLangMenu(), 'switchHtmlLang', 'switchLangMenu')
	...
	langList.forEach(lang=>{
		preLang += 
		`<button onclick="_.lang.replace('${lang}').then(e=>doLangSetup(e));pushNewLang('${lang}');makeSwticher(1,'switchHtmlLang2')" style="width:40px;margin:2px" class="emptybtn">`+
...
Markdown = (mdText, depth = 0, counter = { n: 0 }) => {
	const ATTR_RE = /\s*\{([.\#][^\{\}]*)\}\s*$/;
